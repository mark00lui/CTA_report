# state 檔欄位定義

每支個股一檔：`state/<ticker>.yaml`。台股用數字代號（`3324.yaml`），美股用 ticker（`MSFT.yaml`）。

## 欄位

| 欄位 | 型別 | 說明 |
|---|---|---|
| `ticker` | str | 代號 |
| `name` | str | 中文／英文名 |
| `market` | str | `TW` / `US` |
| `factor_tags` | list | 因子標籤，供 portfolio 層彙總。例：`[ai_capex_hw, optical, cpo]` |
| `thesis` | str | **一句話**論點。寫不成一句話代表論點還沒想清楚。 |
| `thesis_since` | date | 論點成立日，用來看論點壽命 |
| `scenarios` | map | `bear` / `base` / `bull`，各含 `p`、`p_basis`、關鍵財務假設、`eps`、`exit_multiple`、**`multiple_basis`**、`tp`（目標價）、`narrative`（一句話） |
| `key_variables` | list | 決定論點成立與否的變數，**上限 6 個**。每個含 `name` / `value` / `unit` / `tier` / `source` / `updated`。⚠ `updated` 的契約見下方專節 —— 有值卻沒有 as-of 會擋 commit |
| `cta` | map | `位階`（右側確認／整理／破線）、`key_ma`、`support`、`resistance`、`atr20` ＋ `atr20_basis`、`invalidation`（技術面失效價位）＋ `invalidation_basis`、`thesis_review_trigger`、`confirm_trigger`、`price` ＋ `price_basis`、`updated` |

⚠⚠ **`invalidation` 與 `thesis_review_trigger` 是兩個不同的物件，受不同的約束**（2026-09-12 於 8046 分開）：

| 欄位 | 問的是 | 距離約束 |
|---|---|---|
| `invalidation` | 技術位階是否失效 | **必須可被觸發** —— 距現價 ≥2 倍 ATR（下界，擋雜訊）且 ≤20%（上界，擋「太遠所以永遠不會發生」）|
| `thesis_review_trigger` | 論點所依賴的前提是否還成立 | **必須夠深才有意義** —— 不受 ±20% 上界約束 |

**把一個距現價 41% 的價位（8046 的 MA240）塞進 `invalidation`，會讓 ±20% 上界看起來太嚴；真正的問題是那個欄位裝了兩種用途的東西。**

⚠ **`atr20` 是雙邊規則的分母，不可用單日振幅代替** —— 單日振幅的偏誤方向不固定（8299 虛增 2.15 倍、3211 虛增 2.62 倍、3131 虛減 2.69 倍）。
⚠ **下界可由「週收盤」這個時間濾網替代**：週的真實區間約為日 ATR 的 √5 ≈ 2.24 倍，故「週收盤跌破距離 d 的位階」等效於「日收盤跌破距離 2.24d」（2308 的 MA240 1.59 → 3.56 倍；8046 的 MA120 1.30 → 2.91 倍）。
| `signal` | map | `rating`（偏多/中性/偏空）、`conviction`（高/中/低）、`basis`、`changes_if` |
| `falsifiers` | list | 否證點。**可驗證、有日期、有數字門檻**。觸發即強制 L3。 |
| `checkpoints` | list | 未來已知的資訊節點（法說、財報、月營收、產業展會、政策日期），含 `date` / `event` / `what_to_watch` |
| `gaps` | list | 已知的未知。含 `question` / `how_to_close`（取得途徑） |
| `event_log` | list | 事件流水。含 `date` / `level` / `summary`。⚠ **不得有 `delta`** —— 2026-09-13 起由 `validate_state.py` 擋下，理由見下方專節。只留最近 20 筆，更早的壓成月摘要。 |
| `last_updated` | date | |

## `event_log` 不得有 `delta`（2026-09-13 S5）

⚠⚠⚠ **這一條的依據是 schema 自己原本就寫著的「`delta`（一行）」。**

實際做出來的是：**39 檔、250 筆事件、`delta` 合計 409,089 字元，
平均 1,636 字元、最長 4,057（MU 2026-09-12）—— 超出「一行」約 27 倍。**
`event_log` 整體佔 state 檔字元數的中位 **23.8%**（最高 MU 30.1%）。

**而 CLAUDE.md 已經為 `coverage.yaml` 的 `equal_weight_factor_mix[].note`
裁決過完全相同的形狀**：

> 當 state 裡的某個欄位開始承擔「記錄歷史」的職責時，它就站錯地方了。

處置也相同：**搬到 `reports/`，因為只有那裡由 `check_append_only.py` 機械強制不可改。**
⚠ **這一點反直覺但關鍵：搬過去之後它才真正不可改，校準價值是變強，不是變弱。**

### 現在的分工

| 東西 | 住哪裡 |
|---|---|
| 「這次改了什麼」的**一句話** | `state` 的 `event_log[].summary` |
| 「為什麼這樣改」的**完整推論** | **該事件自己的那份報告** |
| 2026-09-13 之前已寫成 `delta` 的歷史 | `reports/2026/09/2026-09-13-<ticker>-event-log-archive.md`（39 份沿革檔，原文逐字） |

**事件的細節本來就寫在該事件的報告裡 —— `delta` 是把它抄了第二份，
而那第二份住在一個可以被事後修飾的檔案裡。**

### 沿革檔的規格

`level: L0`，`signal`／`conviction`／`tp_*` 一律 `n/a` ——
**因為它不新增、不修改、不撤回任何判斷，只搬動文字。**
原文一個字不改、不刪、不摘要。

## `key_variables` 的型別契約（2026-09-13 S3a）

⚠⚠ **在此之前，「這一格該不該是數字」沒有任何地方寫著** ——
於是 `value` 有 **52/232（22%）裝散文**，而那既不算違規也不會被發現。
最尖銳的一例是把時間序列塞進純量：
`value: 16.70（2026Q2）；15.15（FY2025）；11.33（FY2024）` —— **那是三筆 (period, value)。**

### `kind` 必填

| kind | `value` | `unit` |
|---|---|---|
| `量化` | **只能是數值或 `__`** | 實際單位 |
| `質性` | 散文 | **必須寫「質性」**，否則單位在騙人 |

現況：**量化 203 筆／質性 29 筆**。

### `value_qualifier` —— 修飾語必須被表達，不能被抹掉

設計原則：**這組詞彙必須能完整表達原字串的語意，否則轉型就是有損的，
而有損的轉型會把「約 90」變成「90」這種假精確。**

| 詞彙 | 對應原文 | 附加欄位 |
|---|---|---|
| `點值` | 沒有修飾語 | |
| `區間` | `15-20`、`165-169` | **必須**有 `value_low`／`value_high`；`value` 取中點（衍生值，兩端未失） |
| `下界` | 「逾／超過／以上」 | |
| `上界` | 「近／不到／以下」 | |
| `約值` | 「約」 | |
| `序列最新` | 一格裝了時間序列 | **必須**有 `series`；`value` 等於 `series[0].value`（最新的排第一） |
| `多值` | 一格裝了同時存在的多個量 | **必須**有 `components`；`value` 若為數值必須等於其中之一，否則填 `__` |
| `未型別化` | ⚠ **已知債務** | unit 是量化的但 `value` 仍是散文。**2026-09-13 已清為 0**，保留供未來標記 |

### `unit` 是受控詞彙，`unit_note` 裝其餘的一切（2026-09-13 S3d）

232 筆變數曾用了 **108 種相異 `unit`** —— 因為括號裡塞著期間、口徑、甚至額外數據。
收斂的方式是**無損切分**：自第一個括號處切成 `unit`（純單位）＋ `unit_note`（其餘全部），
**一個字都不丟**，再把意義完全相同、只是寫法不同的合併。**結果是 20 種。**

```yaml
unit: '%'
unit_note: （capex／營收，TTM 至 2026-08-02）
```

**受控詞彙（20 種）**

| 類 | 詞彙 |
|---|---|
| 比率 | `%`　`% YoY`　`倍` |
| 台幣 | `元`　`元／股`　`億元新台幣`　`億股`　`新台幣元／櫃` |
| 美元 | `美元`　`美元／股`　`美元／季`　`百萬美元`　`十億美元` |
| 其他 | `年`　`季`　`天`　`類`　`萬套／月`　`TWD/USD`　`質性` |

不在詞彙裡 → **error**。

**已合併的同義詞**：`億元`→`億元新台幣`（22 筆全為 TW）／`x`→`倍`／
`USD mn`→`百萬美元`／`USD bn`→`十億美元`／`USD`→`美元`／
`USD/股`→`美元／股`／`USD/季`→`美元／季`／`TWD`→`元`。

⚠ **`unit_note` 裡仍含數字者有 64 筆**，`validate_state.py` 每次執行都會印出數量。
**那些多半是 `series` 的點或另一個量**（例：`%（2026 H1；去年同期約 83.7%）`
的「去年同期約 83.7%」就是序列的第二點），**是下一批待拆的。**
⚠ **不要用它當永久收納處** —— 它存在的目的是讓 `unit` 先變乾淨，不是讓雜物換個地方放。

### `period` —— 數字描述的期間，與 `updated` 是兩件事

| 欄位 | 問的是 |
|---|---|
| `updated` | **這一格何時被寫進 state** |
| `period` | **這個數字描述的是哪一段時間** |

**兩者長期被混為一談**，而 `period` 因為沒有欄位，被夾帶進 `unit` 字串裡：
`%（2026 Q2）`、`億元新台幣（2026-07）`、`x (FY2027)` ——
**單位與期間混在一格，兩者都無法被驗證。**

```yaml
- name: 毛利率
  kind: 量化
  value: 35.64
  unit: '%'          # 只放單位
  period: 2026 Q2    # 期間有自己的欄位
  tier: 事實
  updated: 2026-09-10   # 這一格何時被寫進 state
```

⚠⚠ **2026-09-13 只拆了括號內是「純期間」的 14 筆。**
`unit` 含括號者共 95 筆，**剩下的 81 筆括號裡混著口徑
（`%（capex／營收）`）、額外數據（`%（Q2 2026；Q1 約 29.5%）`）甚至序列點**——
**逐筆都要判斷，機械拆會把數字摘要掉。**
`validate_state.py` 每次執行都會印出剩餘筆數（`[UNIT]`），讓它是一個會縮小的數字。

⚠ **新寫的變數不要再把期間寫進 `unit`。**

### `series` 與 `components`

```yaml
- name: Azure成長率            # 時間序列
  kind: 量化
  value: 43
  value_qualifier: 序列最新
  series:
  - {period: Q4 FY26, value: 43}       # 最新的排第一，value 就是它
  - {period: FY26 全年, value: 41}
  - {period: Q1 FY27 指引, value: 45}
  unit: '% YoY'

- name: ABF與BT漲價幅度         # 同時存在的多個量，沒有單一代表值
  kind: 量化
  value: __
  value_qualifier: 多值
  components:
  - {name: ABF 載板售價漲幅, value: 50}
  - {name: BT 載板售價漲幅, value: 70}
  unit: '%（2026 全年售價漲幅預估）'
```

⚠⚠ **搬移原則：原字串裡的每一個數字都要有去處，不是被摘要掉。**
去處只有三種：**(a)** 進 `series`／`components`；**(b)** 本來就已經在 `source` 裡；
**(c)** 兩者皆非 → **明寫進 `source` 並註明是型別化時搬過去的**。
2026-09-13 的七筆遷移逐筆按此處理，其中四筆動用了 (c)。

⚠ **`value: __` ＋ `多值` 是一個結論，不是一個沒查到的數字** ——
它說的是「這一格該拆成兩個變數」。8046 的 `key_variables` 已達上限 6，
拆分需先汰換一項，所以它暫時以 `components` 承載。

### ⚠ `未型別化` 是刻意可數的債務，不是分類

**這 7 格的 unit 是 %／元／USD/股／百萬美元 —— 它們本來就是數字，只是被寫成散文。**
標成「質性」會讓一個**未完成的遷移**看起來像**已完成的分類**，
而那正是這一輪在修的那種看不見的債。標成 `未型別化` 則讓它每次驗證都出現。

兩類成因：

| 成因 | 標的 | 為什麼還轉不動 |
|---|---|---|
| `value` 是區塊純量，裡面有大量**未被 `source` 涵蓋**的敘述 | 2308 AI相關營收占比、LITE GAAP淨損的成因 | 轉成數字會毀掉資訊，必須先搬移敘述 |
| `value` 裝的是**時間序列或多個量** | 2308 營益率、6239 市場共識與目標價、8046 ABF與BT漲價幅度、GOOG 股權證券損益對EPS影響、MSFT Azure成長率 | 需要 `series`／`components` 欄位（S3b） |

## `__` 與 `none_qualifies` —— 兩種相反的空（2026-09-13 拆開）

⚠⚠ **在此之前，`__` 同時代表兩種相反的知識狀態**：

| token | 意思 | 該不該算成缺口 |
|---|---|---|
| `__` | **不知道，去查** | **算** —— 它是一筆待辦 |
| `none_qualifies` | **已量化，結論是沒有東西合格** | **不算** —— 它是一個結論 |

**兩者在檔案裡曾經長得一模一樣**，於是 `count_gaps` 把 8 個結論算成 8 個缺口 ——
**「缺口變多」與「查證做得更徹底」在輸出上不可分辨。**
拆開之後未填欄位統計自 89 降為 81。

### `cta.invalidation: none_qualifies` 的成立條件

雙邊規則下**沒有任何位階同時滿足**「距現價 ≥2 倍 ATR」與「≤20%」時使用。

- **必須**同時填 `invalidation_basis`，寫出量到了什麼（哪幾條均線、各距幾倍 ATR、
  區間低點距現價百分之幾）。**沒有依據的 `none_qualifies` 只是缺口換個寫法**，
  `validate_state.py` 會報 **error**。
- ⚠ **它本身就是壓低 `signal.conviction` 的理由** —— 一個無法被技術面否證的位階判斷，
  信心度不該與有失效價位的標的相同。
- ⚠ **不要為了填格子放寬門檻。** 兩個邊界擋的是不同的失效模式，
  而 AXTI 實測過合格帶寬可以是負的（2 倍 ATR 的下界 22.88% 已超出 20% 的上界），
  **那時規則字面不可滿足，`none_qualifies` 是唯一誠實的答案。**

`__` 留在 `invalidation` 上會觸發 **WARN**：那表示還沒量化過。

### 呈現層不顯示 token

`none_qualifies` 是**資料層**的 token（機器可辨、可 grep、不可能與 `__` 混淆）。
`scripts/build_site.py` 的 `invalidationLabel()` 把它譯成讀者看得懂的中文標籤。
**token 在資料層，標籤在呈現層 —— 不要為了好看而把 token 寫成一句中文。**

## `key_variables[].updated` —— as-of 的契約（2026-09-13 嚴格化）

⚠⚠⚠ **在此之前，`validate_state.py` 的 `as_date()` 對解析不出來的值回 `None`，
而呼叫端一律寫成 `if u and ...` —— 於是「填了一個不合法的日期」與「根本沒填」
都變成「跳過」，在輸出上完全看不見。**
**實測 43/232（18.5%）落在這個分支，寬鬆補算後其中 11 筆已逾 90 天，
最舊者 3583「對台積電機台銷貨收入」的 `updated: 2024` 已 986 天 ——
而 `--stale` 在 2026-09-12 回報的是「無陳舊變數」。**

**空白讀起來像「都很新」。一個會靜默跳過的檢查比沒有檢查更危險，因為它讓人以為查過了。**

### 允許的三種形式

| 形式 | 例 | 陳舊判定取的日期 |
|---|---|---|
| 日精度（**預設，YAML 原生 date**） | `updated: 2026-09-11` | 該日 |
| 月精度（字串） | `updated: '2026-08'` | **2026-08-01** |
| 年精度（字串） | `updated: '2024'` | **2024-01-01** |

⚠ **精度不足時一律取該期間的第一天，而那是刻意的下界** ——
它讓陳舊判定偏保守（寧可多報，不可漏報），
**而不是把一個不知道的日子補成看起來比較新的樣子**。
月／年精度合法但會被列進 `COARSE` 區塊，應逐步收斂為日精度。

### 兩條 error（會擋 commit）

1. **`updated` 填了但不是上述任何一種形式** → error。
2. **`value` 有值但 `updated` 未填** → error。
   **沒有 as-of 的數字無法判斷是否過期，等於一個永遠不會被檢查的數字。**
   `value` 也是空的（真正的缺口）則不需要 as-of。

### `--stale` 一定會印涵蓋率

```
涵蓋率：key_variables 共 232 筆，其中 215 筆有 as-of 可判定、17 筆無值且無 as-of（不需判定）
```

⚠ **這一行存在的唯一理由是：讓「涵蓋不全」這件事本身不可能再靜默發生。**
分母對不上時腳本會另外印一行要求去看 ERROR。

## 叢集規則：判準是「整體跨幅」，不是「相鄰間距」

位階群的跨幅小於 1 個 ATR 時，有意義的事件是**穿透整個叢集**而不是觸及它的近端邊緣
（3661 建立）。**而 2026-09-12（MSFT）問出了它的可傳遞性，答案是不可傳遞。**

MSFT 價格下方八條均線的**相鄰間距**（以 ATR 計）是
**0.85 / 0.03 / 0.90 / 0.51 / 0.90** / 4.21 / 0.02 —— 除了一段，每一段都小於 1 個 ATR。
**但 MA120 到 MA50 的整體跨幅是 3.19 個 ATR。**

⚠⚠⚠ **若把規則當成可傳遞的，那六條會併成「一個叢集」，而「穿透整個叢集」就把
`invalidation` 自 −8.47% 推到 −14.70% —— 那完全抵銷了規則的用意。**
規則要防的是「觸及近端邊緣不是一個可分辨的事件」，
**而 0.85–0.90 個 ATR 的間距本來就可分辨（接近一整天的真實區間）。**

**→ 判準是「這一組位階的整體跨幅 < 1 個 ATR」。由低到高貪婪分群：**

```
MSFT 的分群結果
  {MA120 422.76, MA100 430.97, MA200 431.26}  跨幅 0.88 ATR
  {MA60 440.00, MA240 444.93}                 跨幅 0.51 ATR
  {MA50 453.67}                               單獨 → invalidation 取它
  {MA20 494.43, MA5 494.67}                   跨幅 0.02 ATR（兩條都只距現價約 0.1 ATR，無意義）
```

⚠ **此前沒發現這個歧義，是因為前面的實例都剛好整體就小於 1 個 ATR**
（6442 的兩條差 0.134、TSM 的五條整體 0.57、3324 的四條 0.66）——
**MSFT 是第一個「相鄰間距小但整體大」的案例，所以它第一次讓兩種讀法給出不同答案。**

## 除權息回溯調整 —— 算均線之前必做

官方逐日序列**未經除權調整**（已由 3081／3211／2345／3533 四次自算參考價對上第三方報導值證實）。
除息日前的價格因此偏高，**任何跨越除息日的均線都是錯的**，而 2026-09-12 的全清單重校裡
十二檔有十一檔的窗口內含除息日。

**因子 ＝（除息前一日收盤 − 現金股利）÷（1 + 股票股利比例）÷ 除息前一日收盤**
（股票股利 0.10 元 → 比例 0.01，因面額為 10 元）。把因子乘到**除息日之前**的每一根上。

⚠ **方向規則，每次都要用它自我檢查**：調整壓低除息日前的價格，所以
**在價格下方的均線距離會變大**（餘裕改善：2317 的 MA240 自 2.08 → 2.96 倍），
**在價格上方的均線距離會變小**（餘裕變薄：3665 的 MA120 自 2.30 → 2.10 倍）。
**若算出來的方向與此相反，是算錯了，不是例外。**

⚠ ATR20 通常**不受影響** —— 除息日落在 20 日窗口之外時。落在窗口內則必須一併重算。

### 除息日怎麼找：用官方的 `X` 旗標，不要搜尋第三方股利頁

官方逐日序列的**漲跌欄在除權息日會填 `X`**（TWSE 於不計算漲跌價差時如此標記）。
**這是官方自己標的，不需要搜尋，也不會像第三方頁面那樣彼此矛盾**
（3665 的現金額在三個來源間是 15.0／15.184／15.42，且有無股票股利也不一致）。

已對四檔已知除息日驗證全部相符：3665 的 2026-07-15、2317 的 2026-07-02、
3017 的 2025-08-19 與 2026-08-19、3533 的 2026-08-04。

⚠⚠ **但 `X` 有第二個成因：停牌後的復牌日**（前一交易日零成交，漲跌無從計算）。
3665 的 2026-06-11 就是這種 —— 前一日 06-10 因重大訊息待公布而暫停交易。
**→ 偵測除權息時必須排除「零成交日的次一交易日」。** 這個限制是控制組裡剛好有一檔停牌才發現的。

⚠⚠ **而它是必須真的去查的，不是假設性的提醒**：3030 與 3665 **都**在 2026-06-11 帶 X，
**但 3665 的前一日零成交（停牌復牌），3030 的前一日成交 353 萬股（除息）—— 同一日期、兩個成因。**
掃過 16 檔快取齊全的標的，只有這兩檔於該日帶 X，**所以也不是市場級事件，是巧合。**
**「兩檔在同一天出現同一個旗標」會誘使人推論一個共同成因，而這裡沒有 ——
判別依據只有前一交易日的成交量。**

### ⚠⚠⚠ 上櫃（TPEx）不用 X 旗標 —— 要用一致性檢查

**兩個交易所的標記方式不同，而照上市的做法掃上櫃會得到「沒有除權息」這個錯誤結論**（3324 實測）。

| | 除權息日的 `漲跌` 欄 | 能得到什麼 |
|---|---|---|
| **上市（TWSE）** | 填 **`X0.00`** | 只有**日期** |
| **上櫃（TPEx）** | **照常填數字，但那是對「參考價」算的** | **日期 ＋ 參考價** |

**→ 上櫃的偵測法：掃「`漲跌` 欄 ≠ 收盤 − 前一日收盤」的那一天。**
3324 在 14 個月窗口內只有 2026-06-17 不符（前收 1,050.00、收 1,050.00、漲跌欄報 +10.00）。

⚠ **但 TPEx 給的參考價是檔位取整值**：上式反推得 1,040.00，而精確值是 1,038.0967（差 **0.18%**）。
**→ 它是偵測器，不是因子來源。**

✓ **而上櫃有一個上市沒有的好處**：`tpex_mainboard_peratio_analysis` 直接載 **`每股股利`**
（3324 為 **11.90330569**，與公告完全相符到 8 位小數）。
⚠ **那 8 位小數不是雜訊，是「總配發額 ÷ 股數」的結果** —— 3324 原定 12.00 元，
2026-06-01 因執行庫藏股轉讓員工 750,000 股改變流通在外股數而下調。
**所以官方欄位比任何第三方頁面的「12 元」都準。**

⚠ **旗標只給日期，金額仍需另查（上市）。** 兩個官方旁證可用來裁決第三方的不一致：
**官方殖利率 × 收盤 ≈ 現金股利**（3665 的 0.81% × 1,875 ＝ 15.19 → 裁定 15.184；
2317 的 2.89% × 248 ＝ 7.167 → 裁定 7.171；殖利率欄為空則當年無配息，2337 即是），
以及**除息當日若觸及漲停，最高價 ＝ floor(參考價 × 1.1 ÷ 檔位) × 檔位**
（3665 的 1,915.00 因此裁定了股票股利存在）。⚠ 後者的前提是當日確實觸及漲停，
2317 就不適用（參考價的漲停為 264.50，當日最高僅 243.00）。

## 填寫規則

**thesis 要可否證。** 「AI 需求強勁受惠」不是論點，是感想。「液冷從選配轉標配，2026 滲透率決定重評價幅度」是論點 — 因為滲透率可以量測，可以證偽。

**key_variables 上限 6 個。** 超過就代表沒有分清主次。分辨標準：這個變數變動 20%，目標價會不會動 10% 以上？不會的話它不是 key variable。

**scenarios 的機率要有依據。** 不是 25/50/25 的預設值。寫 `p_basis` 說明為何是這個機率（歷史頻率？供應鏈訊號？管理層 guidance 的保守程度？）。

**`multiple_basis` 是必填，且 base 的錨不得是現價。** 每個情境都要寫出 exit multiple 錨在哪：自身歷史區間分位、同業／中週期、reverse-DCF 反推的隱含成長、或成長持續期。

bear 與 bull 錨在「市場實際付過的價」是好做法（52 週低點當時的倍數、題材發酵前的倍數、賣方目標價隱含值），但 **base 用現價隱含倍數等於沒有估值** —— 它會讓加權目標價機械性地貼著現價。

四種錨都拿不到時，`multiple_basis` 寫「採用市場定價，非獨立判斷」，並把 `signal.conviction` 上限壓到「低」。這不是懲罰，是讓「沒有意見」在檔案裡看得見 —— 看不見的無知會被當成判斷使用。

完整規則、循環股框架與常見自欺模式見 `references/valuation.md`。

**falsifiers 要有數字與日期。** 「需求轉弱」不可驗證；「Q3 液冷營收占比未達 35%」可驗證。每支個股至少 2 個否證點，其中至少 1 個在未來 6 個月內可驗證。

**tier 標記不可省略。** 事實／推論／假設／缺口。假設值不得單獨支撐 L2 以上的模型變動。

**本 schema 刻意不含倉位欄位。** 沒有成本、張數、權重、Kelly。若你發現需要記錄尺寸，那屬於這個倉庫之外的事。

**signal 是研究結論，不是交易指示。** `rating` 只有偏多／中性／偏空；`changes_if` 必須是具體價格或事件，不是「持續觀察」。

## 台股專用欄位

`monthly_revenue`：最近 3 個月營收與 YoY，每月 10 日前更新。這是台股最高頻的一手數據，優先度高於任何新聞。
`chips`：三大法人買賣超趨勢、融資餘額變化。可選。

## 因子標籤（供 portfolio 層彙總）

- `ai_capex_hw` — 直接受益於資料中心資本支出的硬體（TSM、3324、6442）
- `ai_platform` — 支出方 / 平台（MSFT、GOOG、META）
- `ai_application` — 應用端貨幣化（INTU）
- `thermal` / `optical` / `cpo` / `foundry` / `advertising` / `saas` — 次產業
- `tw_fx` — 獲利對新台幣匯率敏感
- `single_customer` — 單一客戶占比 > 30%

平台端與硬體端不是分散，是**同一條產業鏈的兩端**：capex 縮手時兩邊一起受傷，只是時序不同。

`state/coverage.yaml` 的因子分析以**等權覆蓋**為基準（每檔 1/n）。那是覆蓋清單的結構描述，與任何實際配置無關 — 本倉庫不記錄配置。

## `valuation_frame` — 估值方法組合

**建檔時決定，每次 `/revalue` 複核。** 判準與處方見 `references/valuation-map.md`。

```yaml
valuation_frame:
  quadrant: Q3
  quadrant_basis: "capex／營收 50%（2026 計畫 500 億元 ÷ 營收預估 998 億元）；每股自由現金流 -14.78 元；cyclical"
  quadrant_since: 2026-09-01
  peer_group: 封測 OSAT
  peers: ["日月光 3711", "京元電 2449", "超豐 2441（⚠ 本檔子公司）"]
  rank_metrics: [先進封裝營收占比, 客戶層級, 資本支出強度與 FCF]
  methods:
    primary: 同業中位數 forward PE（官股投顧同一份報告 23.8／28／42x）
    secondary: 自身歷史 trailing PE（3 年平均 16.5x，現況 28.11x）
    check: Gordon 合理 PB（(ROE-g)/(r-g)）
  axes_crossed: [參照]
  excluded: "⚠ 不做 DCF（Q3 循環股，中週期 FCF 拿不到）；不算 PEG（g 在循環兩端趨近 ±∞）"
  reviewed: 2026-09-01
```

**欄位語意**

| 欄位 | 意義 | ⚠ 常見錯誤 |
|---|---|---|
| `quadrant` | Q1–Q4，決定用哪一類方法 | 建檔後就沒再看過。**象限會漂移** |
| `quadrant_basis` | 判準**要指名數值** | 寫「資本密集」而不寫 capex／營收是多少 |
| `quadrant_since` | 進入該象限的日期 | 漂移時忘了改，於是漂移這件事沒有留下痕跡 |
| `peers` | 可比同業 | **多數族群在覆蓋清單內只有 1–2 檔，必須從清單外取** |
| `rank_metrics` | 2–3 個排序指標 | **事後**挑指標來支持已想好的答案 |
| `methods.secondary` | 副錨 | 留 `__` —— 單一方法無法交錯驗證 |
| `axes_crossed` | 主副錨跨了哪些軸 | 兩個方法都只看自身歷史，**軸④沒動** |
| `excluded` | 該象限**不可用**的方法 | 只寫該用什麼，沒寫不該用什麼 |
| `reviewed` | 最近複核日 | 超過 120 天 `validate_state.py` 會警告 |

⚠ **為何要把「不可以用什麼」也寫下來**：估值方法的誤用多半不是「選錯了主錨」，
而是「順手用了一個這個象限本來就不適用的方法」——
例如對循環股算 PEG、對選擇權價值做 forward-DCF、對 fabless 看 PB。
`excluded` 讓這些禁令跟著檔案走，而不是靠記憶。

⚠ **這一欄是 2026-09-01 引入的，既有檔案本來就沒有。**
`validate_state.py` 對缺漏只發 **NOTE 不發 ERROR**（發 ERROR 會一次擋下全部 commit），
但它每次執行都會列出來——**看不見的無知會被當成判斷使用。**
